    ...
    <>
      ...
      <main className="flex-1 overflow-auto p-6">
        ...
        <div onDragOver={(e) => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onDrop={handleDrop}
          ...
          className={`relative border-2 border-dashed rounded-2xl p-10 text-center cursor-pointer mb-6 transition-all ${dragOver ? "border-blue-400 bg-blue-50 scale-[1.01]" : "border-slate-700 hover:border-slate-300 hover:bg-slate-800"}`}>
          <div className="flex flex-col items-center gap-3">
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-colors ${dragOver ? "bg-blue-100" : "bg-slate-800"}`}>
              <Upload className={`w-5 h-5 ${dragOver ? "text-blue-500" : "text-slate-400"}`} />
            </div>
            <div>
              <p className="text-slate-200 font-semibold text-sm">{dragOver ? "Drop to upload" : "Drop images here or click to browse"}</p>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            {files.map((file) => (
              <div key={file.fileName} className="group bg-slate-900 rounded-2xl border border-slate-100 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                <div className="relative aspect-square bg-slate-800">
                  ...
                  <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-all flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100">
                    <button onClick={() => copyUrl(file.url)} className="w-8 h-8 rounded-lg bg-slate-900/90 flex items-center justify-center text-slate-400 hover:text-blue-600 transition-colors shadow" title="Copy URL">
                      {copiedUrl === file.url ? <CheckCircle className="w-3.5 h-3.5 text-emerald-500" /> : <Copy className="w-3.5 h-3.5" />}
                    </button>
                    <button onClick={() => setDeleteFile(file)} className="w-8 h-8 rounded-lg bg-slate-900/90 flex items-center justify-center text-slate-400 hover:text-red-600 transition-colors shadow" title="Delete">
                      <Trash2 className="w-3.5 h-3.5" />
                    </button>
                  </div>
                </div>
                <div className="p-3">
                  <p className="text-slate-200 text-xs font-medium truncate">{file.fileName}</p>
      ...
      {deleteFile && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 rounded-2xl shadow-2xl p-6 max-w-sm w-full">
            ...
            <h3 className="text-slate-100 font-bold text-lg text-center mb-1">Delete Image?</h3>
            ...
            <div className="flex gap-3">
              <button onClick={() => setDeleteFile(null)} className="flex-1 py-2.5 rounded-xl border border-slate-700 text-slate-400 text-sm font-medium hover:bg-slate-800">Cancel</button>
